...
SCHEDULE_POLL_INTERVAL = int(os.getenv("SCHEDULE_POLL_INTERVAL", "2"))  # Check active schedule blocks every 2 seconds
...
SCHEDULE_ACTIVE_URL = f"http://{BACKEND_SERVER_IP}:{BACKEND_SERVER_PORT}/api/schedule/blocks/active-now"
SCHEDULE_STATUS_URL = f"http://{BACKEND_SERVER_IP}:{BACKEND_SERVER_PORT}/api/schedule/status"
STUDENT_ID   = socket.gethostname()

# Track domains blocked by schedules so we can auto-unblock when window ends.
_scheduled_blocked_domains = set()
...
def get_active_scheduled_domains():
    """Fetch currently active scheduled domains from backend."""
    try:
        response = requests.get(SCHEDULE_ACTIVE_URL, timeout=4)
        if response.status_code != 200:
            return []
        ...
        active_domains = []
        for item in data:
            website = (item or {}).get("website")
            if not website:
                continue
            cleaned = clean_domain(str(website)).lower()
            if cleaned:
                active_domains.append(cleaned)

        # Deduplicate while preserving order.
        return list(dict.fromkeys(active_domains))
    except requests.exceptions.RequestException:
        return []


def report_schedule_status(active_domains, applied_domains, status="ok", last_error=None):
    """Send latest schedule enforcement state to backend for admin visibility."""
    payload = {
    ...
    try:
        requests.post(SCHEDULE_STATUS_URL, json=payload, timeout=4)
    except requests.exceptions.RequestException:
        pass


def sync_scheduled_blocks():
    """Apply newly active schedule blocks and remove expired ones."""
    global _scheduled_blocked_domains

    active_now = set(get_active_scheduled_domains())
    failures = []

    to_block = sorted(active_now - _scheduled_blocked_domains)
    to_unblock = sorted(_scheduled_blocked_domains - active_now)

    if not to_block and not to_unblock:
        report_schedule_status(active_now, _scheduled_blocked_domains, status="ok")
        return

    if to_block:
        print(f"\n📅 Schedule active for {len(to_block)} domain(s): {', '.join(to_block)}")
        for domain in to_block:
            ok = execute_command({
                "action": "BLOCK_DOMAIN",
                "domain": domain,
                "reason": "Scheduled block window",
            })
            if ok:
                _scheduled_blocked_domains.add(domain)
            else:
                failures.append(f"block:{domain}")

    if to_unblock:
        print(f"\n📅 Schedule ended for {len(to_unblock)} domain(s): {', '.join(to_unblock)}")
        for domain in to_unblock:
            ok = execute_command({
                "action": "UNBLOCK_DOMAIN",
                "domain": domain,
            })
            if ok and domain in _scheduled_blocked_domains:
                _scheduled_blocked_domains.remove(domain)
            elif not ok:
                failures.append(f"unblock:{domain}")

    report_schedule_status(
        active_now,
        _scheduled_blocked_domains,
...
def block_via_firewall(domain, ip_addresses):
    ...
    for ip in ip_addresses:
        for direction in ["out", "in"]:
            # Separate rule names for outbound and inbound so unblock can remove both
            rule_name = f"Block_{domain}_{ip}" if direction == "out" else f"Block_{domain}_{ip}_in"
            cmd = [
                "netsh", "advfirewall", "firewall", "add", "rule",
                f"name={rule_name}", f"dir={direction}", "action=block",
                f"remoteip={ip}", "protocol=any", "profile=any"
            ]
            try:
                ...
                if result.returncode == 0:
                    blocked += 1
                    print(f"    ✅ Firewall rule added: block {ip} ({direction})")
                    ...
                    print(f"    ℹ️  Rule already exists for {ip} ({direction})")
                    blocked += 1
                else:
                    print(f"    ⚠️  Could not block {ip} ({direction}): {out}")
            except subprocess.TimeoutExpired:
                print(f"    ⏱️  Timeout adding rule for {ip} ({direction})")
    return blocked
...
def unblock_via_hosts(domain):
    hosts_path = r"C:\Windows\System32\drivers\etc\hosts"
    # Generate variants like the blocking function does
    variants = [domain] if domain.startswith("www.") else [domain, f"www.{domain}"]
    try:
        with open(hosts_path, "r") as f:
            lines = f.readlines()
        
        # Remove lines that match ANY variant (prevents partial matches)
        filtered = []
        for ln in lines:
            should_keep = True
            for variant in variants:
                if f" {variant}" in ln or f"\t{variant}" in ln:
                    should_keep = False
                    break
            if should_keep:
                filtered.append(ln)
        ...
        if removed_count > 0:
            ...
            print(f"    ✅ Hosts file: removed {removed_count} line(s) for {domain} and variants")
        else:
...
def execute_command(command):
    ...
    if action == "BLOCK_DOMAIN":
        ...
        if domain:
            return block_domain_local(domain, reason)
        else:
        ...
        domain = command.get("domain")
        if domain:
            return unblock_domain_local(domain)
        else:
...
def main():
    ...
    print(f"⏱️  Send every : {SEND_INTERVAL}s  |  Poll every: {POLL_INTERVAL}s  |  Schedule poll: {SCHEDULE_POLL_INTERVAL}s")
    print()
    print("🔍 Monitoring:")
    ...
    print("   ✅ Scheduled BLOCK / UNBLOCK from dashboard")
    print()
    if platform.system() == "Windows":
    ...
    last_send_time = 0
    last_poll_time = 0
    last_schedule_poll_time = 0

    while True:
        try:
            current_time = time.time()

            # Keep schedule enforcement responsive from startup.
            if last_schedule_poll_time == 0:
                sync_scheduled_blocks()
                last_schedule_poll_time = current_time
            ...
            # Poll active schedule blocks and keep student enforcement in sync.
            if current_time - last_schedule_poll_time >= SCHEDULE_POLL_INTERVAL:
                sync_scheduled_blocks()
                last_schedule_poll_time = current_time
